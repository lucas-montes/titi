...
for (var t = 0; t < tc.length; t++) {
  tc[t].setAttribute('src', 'https://ik.imagekit.io/smartlinker/images/logo-dark.svg');
}
...
function getAndSetAttribute(selectors, image) {
  var element = document.querySelector(selectors);
  if (element) {
    element.setAttribute('src', 'https://ik.imagekit.io/smartlinker/images/' + image);
  }
}
// TODO: watch ou here. is this really used? this could break as we import images from remote now
function setLogo(logo) {
  getAndSetAttribute('.pc-sidebar .m-header .logo-lg', logo);
  getAndSetAttribute('.navbar-brand .logo-lg', logo);
  getAndSetAttribute('.navbar-brand .brand-logo', logo);
  getAndSetAttribute('.auth-main .brand-logo', logo);
  getAndSetAttribute('.footer-top .footer-logo', logo);
}
